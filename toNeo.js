...
const processInfoBox = () => {
    //Flat map of commands got through processInfoField
}
const processInfoField = ({parent, field}) =>
{   
    const {field_model, name} = field
    if (field_model == "location") return []
    
    let command
    //For entity obtain through localData needed info 
    if (field_model == "entity") return [`MERGE (${parent})-[]->(e1: )`]
    else {
        return `SET ${parent}.${name}`
    }
...
const create_entry = async(incoming_entry) => {
    ...
    try {
      ...
      const writeQuery = `MERGE (c1:Company {id: $id})
                          SET c1.name = $name
                          SET c1.slug = $slug
                          RETURN c1`
